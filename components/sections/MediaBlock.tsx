...
export default function MediaBlock({ content }: { content: MediaBlockContent }) {
  ...
  return (
    <SectionWrapper className="px-6 py-8 bg-[var(--color-background)]">
      <div className="max-w-6xl mx-auto w-full rounded-[var(--radius-lg)] overflow-hidden shadow-[var(--shadow-lg)]">
        ...
        {content.type === 'placeholder' && (
          <div className={`w-full ${aspect} flex items-center justify-center bg-[var(--color-surface)]`}>
            <div className="flex flex-col items-center gap-3 text-[var(--color-text-primary)]/20">
              ...
              <span className="font-body text-sm">{content.alt ?? 'Product screenshot'}</span>
            </div>
          </div>
        )}
      </div>
